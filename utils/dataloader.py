...
class SSDDataset(Dataset):
    ...
    def get_random_data(self, annotation_line, input_shape, jitter=.3, hue=.1, sat=1.5, val=1.5):
        ...
        image3 = Image.fromarray(cv2.cvtColor(image2, cv2.COLOR_BGR2RGB))

        # 放置图片
        dx = int(self.rand(0, w - w))
        dy = int(self.rand(0, h - h))


        new_image = Image.new('RGB', (w, h),
                              (np.random.randint(0, 255), np.random.randint(0, 255), np.random.randint(0, 255)))
        new_image.paste(image3, (dx, dy))
        image = new_image
        ...
        if len(box) > 0:
            ...
            box[:, [0, 2]] = box[:, [0, 2]] * w / iw + dx
            box[:, [1, 3]] = box[:, [1, 3]] * h / ih + dy
            if flip:
                box[:, [0, 2]] = w - box[:, [2, 0]]
            box[:, 0:2][box[:, 0:2] < 0] = 0
            box[:, 2][box[:, 2] > w] = w
